...
import 'package:rafeed_provider/ServiceProviderApp/screen/LoadingScreen.dart';
...
import 'ServiceProviderApp/Providers/provider.dart';
...
Future<void> main() async {
  ...
  await ShP.shp.initShP();
  final res = await http.get(Uri.parse("${server}rest/user/categories"));
  if (res.statusCode == 200) {
    var data = jsonDecode(res.body);
    categories = (data as List).map((e) => Category.fromMap(e)).toList();
  }
  runApp(MultiProvider(
    providers: [
      ...
      ChangeNotifierProvider<LoginSignup>(
        create: (_) => LoginSignup(),
      ),
      ChangeNotifierProvider<ProviderService>(
        create: (_) => ProviderService(),
      ),
      ChangeNotifierProvider<ServicesService>(
        create: (_) => ServicesService(),
      ),
