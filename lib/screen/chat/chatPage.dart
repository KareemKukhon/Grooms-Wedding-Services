...
import 'package:Rafeed/screen/chat/messageCard.dart';
...
class _ChatPageState extends State<ChatPage> {
  ...
  Widget build(BuildContext context) {
    UserModel providerModel = Provider.of<LoginSignup>(context).user!;
    return Scaffold(
        appBar: AppBar(
            ...
            title: Row(
              ...
              children: [
                ...
                CircleAvatar(
                  radius: 16.r,
                  backgroundImage:
                      NetworkImage(server + '/' + widget.recieverLogo),
                ),
              ],
            ),
                            ...
                            children: [
                              ...
                              InkWell(
                                onTap: () async {
                                  ...
                                  final res = await Provider.of<ChatService>(
                                          context,
                                          listen: false)
                                      .sendMessage(map, providerModel.id ?? "");
                                  if (res != null) {
                                    var chat = jsonDecode(res.body);
                                    provider.updateMessages(ChatModel.fromMap(
                                        chat, provider.user!.id!));
